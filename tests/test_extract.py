...
def test_extraction():
    ...
    tooFewColumns = re.compile(r"^([^,]*,){1,3}[^,]*$") #Matches any row with exactly 3 commas. 1, 2, or 3 commas. This is too few!
    tooManyColumns = re.compile(r"^([^,]*,){5,10}[^,]*$") #Matches any row with exactly 5,6,7,8,9, or 10 commas. Too many! We should have exactly 4 in each line.
